const input = `gbcefa eac acfbg ae dcabfg begcdaf ecgba fgaedc beaf gcbde | cbgfa gedcb fgecab fbagdc
...
const arr = input.split('\n').map(el => {
  const alala = el.split(' | ')
  return {
    pattern: alala[0].split(' ').map(a => a.split('').sort()),
    output: alala[1].split(' ').map(a => a.split('').sort()),
  }
})

const getDict = (pattern) => {
  const dict = []
  dict[1] = pattern.find(el => el.length === 2)
  dict[4] = pattern.find(el => el.length === 4)
  dict[7] = pattern.find(el => el.length === 3)
  dict[8] = pattern.find(el => el.length === 7)
  dict[6] = pattern.find(el => {
    if (el.length !== 6) {
      return false
    }
    let n = 0
    el.forEach((val6) => {
      if (dict[1].some(val1 => val1 === val6)) {
        n++
      }
    })
    return n === 1
  })
  dict[0] = pattern.find(el => {
    if (el.length !== 6 || el.join('') === dict[6].join('')) {
      return false
    }
    let n = 0
    el.forEach((val6) => {
      if (dict[4].some(val4 => val4 === val6)) {
        n++
      }
    })
    return n === 3
  })
  dict[9] = pattern.find(el => {
    if (el.length !== 6 || el.join('') === dict[6].join('') || el.join('') === dict[0].join('')) {
      return false
    }
    return true
  })
  dict[3] = pattern.find(el => {
    if (el.length !== 5) {
      return false
    }
    let n = 0
  ...
  const top = dict[1].find(letter => dict[6].indexOf(letter) === -1)
  dict[2] = pattern.find(el => {
    if (el.length !== 5 || el.join('') === dict[3].join('')) {
      return false
    }
    let n = 0
    el.forEach((val6) => {
  ...
  dict[5] = pattern.find(el => {
    if (el.length !== 5 || el.join('') === dict[2].join('') || el.join('') === dict[3].join('')) {
      return false
    }
    return true
  })
...
const res = arr.reduce((acc, cur) => {
  const dict = getDict(cur.pattern).map(el => el.join())
  const val = +cur.output.map(el => dict.indexOf(el.join())).join('')
  return acc + val
}, 0)
